...
- (void)upPayloadTextIndex
{
    ...
    if (range.location != NSNotFound)
    {
        ...
        NSString *alertStr = [NSString stringWithFormat:@"%@%lu%@", before, value, after];
        
        NSRange ran1 = [alertStr rangeOfString:@"\"badge\":"];
        
        if (ran1.location != NSNotFound)
        {
            NSString *sub1 = [alertStr substringToIndex:ran1.location+ran1.length];
            
            NSRange ran2 = [alertStr rangeOfString:@",\"sound\":\"default\"}}"];
            
            if (ran2.location != NSNotFound)
            {
                NSString *sub2 = [alertStr substringFromIndex:ran2.location];
                
                NSString *sendJson = [NSString stringWithFormat:@"%@%ld%@",sub1,value,sub2];
                
                _payloadField.string = sendJson;
                
                NSLog(@"llpp-- %@ --- %@",sub1,sub2);
